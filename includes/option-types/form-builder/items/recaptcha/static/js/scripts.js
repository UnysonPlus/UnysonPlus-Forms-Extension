fw.shortcodesLoadData().then(function ( response ) {
  fwEvents.on('fw-builder:'+ 'form-builder' +':register-items', function(builder){
    ...
    var ItemView = builder.classes.ItemView.extend({
      template: fw.template(
        ...
        '<div class="fw-form-item-preview-label-wrapper"><label data-hover-tip="<%- edit_label %>"><%- label %></label> </div>'+
        '<div class="fw-form-item-preview-label-edit"><!-- --></div>'+
        '</div>'+
        '<div class="fw-row fw-form-item-recaptcha-preview">'+
        '<div class="alpha fw-col-xs-6">'+
        '<div class="fw-form-item-preview-input"><input type="text" placeholder="<%- site_key %>" value="<%- default_value %>"></div>'+
        '</div>'+
        '<div class="omega fw-col-xs-6">'+
        '<div class="fw-form-item-preview-input"><input type="text" placeholder="<%- secret_key %>" value="<%- default_value %>"></div>'+
        '</div>'+
        '</div>'+
        '</div>'+
        '</div>'
      ...
      render: function () {
        this.defaultRender({
          ...
          site_key: fw.opg('recaptcha/site-key', this.model.get('options'))
            ? fw.opg('recaptcha/site-key', this.model.get('options'))
            : localized.l10n.site_key,
          secret_key: fw.opg('recaptcha/secret-key', this.model.get('options'))
            ? fw.opg('recaptcha/secret-key', this.model.get('options'))
